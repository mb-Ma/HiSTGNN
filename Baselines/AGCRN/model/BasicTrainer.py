...
class Trainer(object):
    ...
    def val_epoch(self, epoch, val_dataloader):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                label = target[..., :self.args.output_dim][:, :, :4, :]
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            data = data[..., :self.args.input_dim]
            label = target[..., :self.args.output_dim][:, :, :4, :]  # (..., 1)
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        y_true = torch.cat(y_true, dim=0)[:, :, :4, :]
        y_pred = scaler.inverse_transform2(torch.cat(y_pred, dim=0))
        ...
        for i in range(y_true.shape[2]):
            mae, rmse, mape, _, _ = All_Metrics(y_pred[:, :, i, :], y_true[:, :, i, :],
                                                args.mae_thresh, args.mape_thresh)
            a_mae.append(mae.cpu().numpy())
            a_rmse.append(rmse.cpu().numpy())
        logger.info("Average variable, MAE: {:.2f}, RMSE: {:.2f}, MAPE: {:.4f}%".format(
            np.mean(np.array(a_mae)), np.mean(np.array(a_rmse)), 0.0))
